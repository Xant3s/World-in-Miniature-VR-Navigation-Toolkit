...
// The custom inspector. Displays only relevant settings.
    ...
    public class MiniatureModelEditor : Editor {
        ...
        public override VisualElement CreateInspectorGUI() {
            var configField = new ObjectField(label:"Configuration") {
                allowSceneObjects = false,
                objectType = typeof(WIMConfiguration),
                bindingPath = nameof(MiniatureModel.Configuration)
            };
            //configField.RegisterCallback<ChangeEvent<UnityEngine.Object>>((e) => {
            //    Debug.Log("update");
            //    //root.Q<HelpBoxTest>(name: "config-missing").style.display = !WIM.Configuration ? DisplayStyle.Flex : DisplayStyle.None;
            //    //root.Q<VisualElement>("master-container").style.display = WIM.Configuration ? DisplayStyle.Flex : DisplayStyle.None;
            //});
            root.Q<VisualElement>("ConfigurationContainer").Add(configField);
            root.Q<HelpBoxTest>(name: "config-missing").style.display = !WIM.Configuration ? DisplayStyle.Flex : DisplayStyle.None;
            root.Q<VisualElement>("master-container").style.display = WIM.Configuration ? DisplayStyle.Flex : DisplayStyle.None;
            ...
            Action action = () => {
                ...
                if(!WIM.Configuration) {
                    ...
                    //    "WIM configuration missing. Create a WIM configuration asset and add it to the Miniature Model script.",
                    //    MessageType.Error);
                    return;
                }
